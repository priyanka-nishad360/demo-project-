...
import PageContainer from "../components/pageLayouts/PageContainer.jsx";
...
const Career = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (size > maxlimit && type !== "application/pdf") {
      throw new Error("Only Pdf File Accepted and Should be less than 100 kb.");
    }
    setSuccess(false);
    setMessage("");
    setLoading(true);
    ...
    formData.append("skills", "see resume");
    ...
    if (!res.ok) {
      setSuccess(false);
      toast("Error in submiting your form", {
        type: "error",
      });
    } else {
