...
const insurancePlansData = [
  {
    id: 7,
    name: 'Basic Plan',
    title : 'Basic Plan Car Insurance',
  ...
  },
  {
    id: 8,
    name: 'Standard Plan',
    title : 'Standard Plan Car Insurance',
  ...
  },
  {
    id: 9,
    name: 'Premium Plan',
    title : 'Premium Plan Car Insurance',
...
const CarInsurancePlan = () => {
  ...
  useEffect(() => {
    const prevCartData = JSON.parse(localStorage.getItem('apiCart')) || [];
    setCart(prevCartData);
    const InsuranceInfo = JSON.parse(localStorage.getItem('InsuranceInfo')) || {};
    setInfo(InsuranceInfo);

    const isItemInCart = prevCartData.some((item) => item.insuranceCategory === 'Car Insurance');
  ...
  const handleAddToCart = (plan) => {
    ...
    if (existingItemIndex !== -1) {
      ...
      updatedCart.push({ ...plan,link : '/select_car_insurance_plan', insuranceCategory: info });
    }
  ...
  const handleRemoveFromCart = (plan) => {
    ...
    const isItemInCart = updatedCart.some((item) => item.insuranceCategory === 'Car Insurance');
    setIsAnyItemInCart(isItemInCart);
  };
...
export default CarInsurancePlan;
