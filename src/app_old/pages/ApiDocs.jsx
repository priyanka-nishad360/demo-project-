...
export default function ApiDocs({ params }) {
  const pathURL = {
    slug: params.slug[0],
    apiName: params.slug[1],
  };
  ...
  const existsInCart = cart.findIndex((i)=>( i.title.replace(/ /g, "").toLowerCase() === pathURL.apiName))
  ...
  useEffect(() => {
    ...
    prevData ? setPrevCartData(prevData) : setPrevCartData([]);

    prevData?.filter((item, i) => item.title === apiData?.title).length > 0
  ...
  const addToCartHandler = () => {
    try {
      ...
      const updatedCartData = [...prevCartData, data];
      localStorage.setItem("apiCart", JSON.stringify(updatedCartData));
      dispatch({ type: API_CART, payload: updatedCartData });

      setPrevCartData(updatedCartData); // Update prevCartData to include the newly added item
      setAlreadyInCart(true);
    } catch (err) {
      console.error(err);
    }
  ...
  const removeFromCartHandler = () => {
    try {
      if (alreadyInCart) {
        const newCartData = prevCartData.filter(
          (item) => item.title !== data.title
        );
        localStorage.setItem("apiCart", JSON.stringify(newCartData));
        dispatch({ type: API_CART, payload: newCartData });

        setPrevCartData(newCartData); // Update prevCartData without the removed item
        setAlreadyInCart(false);
      }
    } catch (err) {
      console.error(err);
      ...
      {data ? (
        <div className="max-w-5xl mx-auto my-10">
          ...
          <div className="mt-8">
            ...
            <p className="description mt-2">{data.overview}</p>
          </div>
          <div className="flex p-3 mt-5 bg-[#f0f0f1] rounded font-semibold justify-between items-center">
            <div className="flex text-2xl ">
              ...
              <div className="ml-2 heading-4">₹ {data.price} monthly</div>
            </div>
            <div>
              {/* add to cart button is used on both here just for styling it does not affect the logic of the button */}
                  {existsInCart!==-1 ? (
                    <button
                ...
                {data.queryParams && (
                  <div className="mt-8">
                    ...
                    <table className="table-auto border w-full mt-2 text-sm">
                      ...
                      <tbody>
                        {data.queryParams.map((element, i) => (
                          <tr
                            key={i}
                            className="text-left odd:bg-white even:bg-zinc-100"
                          >
