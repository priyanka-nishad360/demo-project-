...
const CreateSale = ({ currentRow, onClose, onRefresh }) => {
    ...
    <>
      ...
      >
        <div className="grid md:grid-cols-3 gap-6">
          {Object.keys(createSaleInputs).map((key) => (
            <>
              <div key={createSaleInputs[key].id}>
                {createSaleInputs[key].type === 'select' ? (
                  <Input
                    {...register(key)}
                    {...createSaleInputs[key]}
                    value={createSaleInputs[key].options.find(
        ...
        <div className="flex justify-center w-full my-6">
          ...
          >
            {isLoading ? (
              ...
              <>{currentRow ? 'Update Sale' : 'Create Sale'}</>
            )}
          </Button>
        </div>
      </form>
